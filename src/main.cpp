      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
            ...
            if (car_ahead_distance < 20.0)
                set_speed = min(speed_limit, car_ahead_speed);



            double dist_inc = set_speed*0.02;
                for(int i = 0; i < 50; i++)
                {
                    vector<double> point2 = getXY(car_s+(dist_inc*i), 6.0, map_waypoints_s, map_waypoints_x, map_waypoints_y);
                    next_x_vals.push_back(point2[0]);
                    next_y_vals.push_back(point2[1]);
                }
